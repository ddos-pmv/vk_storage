...
KVStorage<Clock>::KVStorage(Clock clock) : clock_(clock), key_index_(BucketTraits(buckets_.data(), buckets_.size())) {}

template<ClockType Clock>
void KVStorage<Clock>::store(std::span<std::tuple<std::string, std::string, uint32_t>> entries) {
...
void KVStorage<Clock>::set(std::string key, std::string value, uint32_t ttl) {
    ...
    if (existing != key_index_.end()) {
        ...
        auto expiry = (ttl > 0) ? clock_.now() + std::chrono::seconds(ttl) : typename Clock::time_point{};
...
template<ClockType Clock>
std::vector<std::pair<std::string, std::string>> KVStorage<Clock>::getManySorted(std::string_view key,
...
template<ClockType Clock>
typename KVStorage<Clock>::Entry *KVStorage<Clock>::create_entry(std::string key, std::string value,
